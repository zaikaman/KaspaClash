...
export default function BotBetHistoryPage() {
    const [history, setHistory] = useState<BotBetHistoryItem[]>([]);
    ...
    async function fetchHistory() {
        ...
        try {
            ...
            const response = await fetch(
                `/api/bot-betting/history?address=${address}&limit=${ITEMS_PER_PAGE}&offset=${offset}`
            );
                    ...
                    <div className="flex items-center justify-center min-h-[60vh]">
                        <div className="text-center">
                            ...
                            <p className="text-cyber-gray mb-6">
                                Connect your wallet to view your bot betting history
                            </p>
                            <Link href="/spectate">
            ...
            <div className="container mx-auto px-4 py-8 max-w-7xl">
                ...
                {!loading && !error && history.length === 0 && (
                    <div className="bg-black/40 border border-orange-500/40 rounded-lg p-12 text-center">
                        ...
                        <p className="text-sm text-gray-500 mt-2">Place your first bet on a bot match!</p>
                        <Link href="/spectate">
                    ...
                    <>
                        <div className="space-y-4">
                            {history.map((bet, index) => {
                                const bot1Character = getCharacter(bet.bot1CharacterId);
                                const bot2Character = getCharacter(bet.bot2CharacterId);
                                const betOnBot1 = bet.betOn === 'bot1';
                                    ...
                                    >
                                        <div className="flex flex-col md:flex-row gap-6">
                                            ...
                                            <div className="flex items-center gap-4 flex-1">
                                                {/* Bot 1 */}
                                                <div className={`flex-1 text-center ${betOnBot1 ? 'opacity-100' : 'opacity-50'}`}>
                                                        ...
                                                        {bot1Character && (
                                                            <img
                                                                ...
                                                                className={`w-full h-full object-cover rounded-lg border ${
                                                                    betOnBot1 ? 'border-orange-500' : 'border-gray-500'
                                                                }`}
                                                            />
                                                        )}
                                                        {betOnBot1 && (
                                                    ...
                                                    <p className="text-white font-orbitron text-sm">{bet.bot1Name}</p>
                                                    {bet.winner === 'bot1' && (
                                                ...
                                                <div className={`flex-1 text-center ${!betOnBot1 ? 'opacity-100' : 'opacity-50'}`}>
                                                    ...
                                                    <p className="text-white font-orbitron text-sm">{bet.bot2Name}</p>
                                                    {bet.winner === 'bot2' && (
                                                        <p className="text-green-400 text-xs mt-1">Winner</p>
                                                    )}
                                                </div>
