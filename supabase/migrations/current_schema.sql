...
CREATE TABLE public.fight_state_snapshots (
  ...
  player1_current_animation text NOT NULL DEFAULT 'idle'::text,
  ...
  player2_current_animation text NOT NULL DEFAULT 'idle'::text,
  ...
  CONSTRAINT fight_state_snapshots_match_id_fkey FOREIGN KEY (match_id) REFERENCES public.matches(id)
);
...
CREATE TABLE public.rounds (
  ...
  animation_phase text CHECK (animation_phase IS NULL OR (animation_phase = ANY (ARRAY['none'::text, 'running_to_center'::text, 'p1_attacking'::text, 'p2_attacking'::text, 'both_attacking'::text, 'running_back'::text, 'round_end_death'::text, 'round_end_text'::text, 'round_end_countdown'::text]))),
  animation_started_at timestamp with time zone,
  animation_duration_ms integer DEFAULT 0,
  player1_current_animation text DEFAULT 'idle'::text,
  player2_current_animation text DEFAULT 'idle'::text,
  player1_animation_started_at timestamp with time zone,
  player2_animation_started_at timestamp with time zone,
  player1_is_stunned boolean DEFAULT false,
  player2_is_stunned boolean DEFAULT false,
  player1_energy_drained integer DEFAULT 0 CHECK (player1_energy_drained >= 0),
  player2_energy_drained integer DEFAULT 0 CHECK (player2_energy_drained >= 0),
  player1_hp_regen integer DEFAULT 0 CHECK (player1_hp_regen >= 0),
  player2_hp_regen integer DEFAULT 0 CHECK (player2_hp_regen >= 0),
  player1_lifesteal integer DEFAULT 0 CHECK (player1_lifesteal >= 0),
  player2_lifesteal integer DEFAULT 0 CHECK (player2_lifesteal >= 0),
  player1_outcome text CHECK (player1_outcome = ANY (ARRAY['hit'::text, 'blocked'::text, 'stunned'::text, 'staggered'::text, 'reflected'::text, 'shattered'::text, 'missed'::text, 'guarding'::text])),
  player2_outcome text CHECK (player2_outcome = ANY (ARRAY['hit'::text, 'blocked'::text, 'stunned'::text, 'staggered'::text, 'reflected'::text, 'shattered'::text, 'missed'::text, 'guarding'::text])),
